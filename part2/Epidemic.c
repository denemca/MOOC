...
Output:
2
For a total population of 10 inhabitants, on day 1 a single person is
infected. This is followed by 2 new people on the second day for a total
of 3 infected people. On the third day,  6 new people are infected for a
total of 9 infected people. On the fourth day the last of the 10 people
is infected (though the epedemic had the potential to infect 18 people on
the fourth day) so your program should output '4'.
...
int main(void){
    ...
    int currentPopulation, days = 1, infected = 1, temp;
    scanf("%d", &currentPopulation);

    while(currentPopulation > infected){
        temp = 2*infected;
        infected = infected+temp;
        days++;
        printf("On day %d, %d were infected\n", days, infected);
    }
