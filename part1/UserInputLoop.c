...
int main(void){
    int i, enginePower, resistance, weight, height, numberOfRobots;
    int total = 0;
    // printf("Enter number of robots: ");
    scanf("%d", &numberOfRobots);
    for(i = 0; i < numberOfRobots; i++){
        // printf("Enter height, weight, enginePower, and resistance: ");
        scanf("%d%d%d%d", &height, &weight, &enginePower, &resistance);
        total += (enginePower + resistance) * (weight - height);
    }
    printf("%d", total);
    return 0;
}